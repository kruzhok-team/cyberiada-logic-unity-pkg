...
* ```IEdgeView``` - представления ребра
* ```IEdgeActionView``` представление поведение ребра
### 2. Реализация фабрики представлений элементов графа
Необходимо реализовать интерфейс ```IGraphElementViewFactory```. Данный интерфейс позволяет управлять циклом жизни создаваемых представлений: создавать и удалять представляния элементов графа. Список методов, реализуемых данным интерфейсом:
...
Для того, чтобы задать события, поведения и условия, которые будет поддерживаться целевой платформой, необходимо создать источник контекста выполнения.
...
*  ```IEnumerable<string> GetActions()``` - возвращает все поведение, поддерживаемые интерпретатором
*  ```IEnumerable<string> GetVariables()``` - возвращает все переменные, поддерживаемые интерпретатором
### 4. Создание клиентского класса ```TestGraphEditor```
В качестве зависимости в такой ```TestGraphEditor``` необходимо передать ранее созданную фабрику представлений ```IGraphElementViewFactory``` и источник контекста выполнения ```IExecutionContextSource```, а также ```GraphEditor``` и любой ```CyberiadaGraph```. В нем необходимо написать методы создание и удаления элементов графа, делегировав их ```GraphEditor```. 
Редактирование элементов графа можно реализовать через использование методов ```SetCondition```, ```SetTrigger```, ```SetParameters```, которые находятся внутри интерфейсов представления элементов графа. Для добавления возможности выбора редактируемого значения можно воспользоваться ```IExecutionContextSource```.
